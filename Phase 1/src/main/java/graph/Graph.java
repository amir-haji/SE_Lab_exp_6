...
public class Graph {
    ...
    public void bfs(Node s, Node hate) {
        this.resetVisits();
        ...
        while (!nodes.isEmpty()) {
            ...
            if (!frontNode.isVisited() && !frontNode.equals(hate)) {
    ...
    public void dijkstra(Node s, Node hate) {
        this.resetVisits();
    ...
    public boolean canTravelWithoutHatedCity(Node start, Node end, Node hate) {
        if (start.equals(hate) || end.equals(hate)) {
            return false;
        }
        this.bfs(start, hate);
